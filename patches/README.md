...
# 0.5) Windows 中文目錄閃退修正（只影響 Windows build；其他平台無作用）
git apply ../freesynd-cht/patches/05-win-utf8-manifest.patch
...
# 1.6) 簡報畫面卡死修正（強烈建議；CJK 折行 bug，~28 關會中）
git apply ../freesynd-cht/patches/06-fix-briefing-hang.patch

# 1.7) 裝備畫面「重新裝填」鈕在滿彈時改顯示反灰（建議；UX）
git apply ../freesynd-cht/patches/07-reload-button-greyed.patch
...
| `05-win-utf8-manifest.patch` | 3 檔 | **Windows 專屬**：把遊戲放在**中文資料夾**下會閃退。中文（Big5/CP950）Windows 的 narrow `fopen`/`ifstream` 會把 UTF-8 路徑當 Big5 解讀 → 找不到 `data`/設定檔 → `ConfigFile::file_not_found` 未捕捉 → terminate。嵌入 `<activeCodePage>UTF-8</activeCodePage>` 應用程式 manifest（Win10 1903+）讓行程 ANSI 碼頁＝UTF-8。以 `windres` 把 manifest 編進 exe。其他平台無作用。已在 Wine 強制 ACP=950 + 中文路徑下重現並驗證修好 |
| `06-fix-briefing-hang.patch` | 1 檔 | **任務簡報畫面卡死**(灰白凍結):`BriefMenu::read_next_word` 只在空格/換行斷行,但 CJK 無空格 → 整行硬斷行(~20 全形字)在 SIZE_2 ×2(24px/字)超過 470px 寬度預算,卻**不消耗文字** → `update_briefing_text` 在每頁開頭 `continue` **無窮迴圈**。英文有空格不會中;模擬顯示 **28/50 關**會卡(蒙古/伊朗/東歐…)。修法:過寬的單詞在空行時以 UTF-8 codepoint 邊界**字元級斷行**,保證消耗 ≥1 字。平台無關;實機驗證 50 關全部正常 |
| `07-reload-button-greyed.patch` | 5 檔 | **裝備畫面「重新裝填」鈕 UX**:原版只在武器**彈藥未滿**時顯示該鈕,滿彈時整顆消失(只剩取消/賣出),老玩家誤以為是 bug。改成**只要選到自己擁有的可用武器就一直顯示**,彈藥滿時渲染成**反灰(disabled)**。引擎面:選單字型的 CJK 字無明暗變體、`Option::draw` 又不看 `enabled_`,故新增**染色版 `MenuFont::drawText`**(`setColorModulation`,畫完還原白)+ `MenuText::drawGreyed()`,`Option::draw` 在 `!isWidgetEnabled()` 時走灰色(通用修正:disabled 的接受鈕也一併反灰)。`selectmenu` 改 `setVisible(可用)` + `setWidgetEnabled(需裝填)` |
