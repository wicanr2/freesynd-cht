...
這份紀錄可以**三層讀**：想直接玩 → 跳到 [⚡ 快速開始](#quickstart)；想當複習 → 從頭把每章讀完；
想當資料庫 → [🔧 技術筆記](#tech)、[Syndicate AI 邏輯考古](docs/syndicate-ai-logic.md)、[術語對照 CONTEXT.md](CONTEXT.md) 各取所需。
...
上一章你在出擊前替幹員配好了義體與 IPA——但那三條滑桿要到**戰場上敵我面對面**那一刻才真正見真章。
...
一場任務的生死由戰場上的 AI 決定，但《Syndicate》真正的野心是整顆地球——下了戰場，戰略地圖才是這盤棋的全貌。
...
- **譯名考據**：**廖奇建**先生於《軟體世界》第 61 期（1994）的攻略，是「極道梟雄」這個台灣譯名的來源，也是本專案命名與譯名考古的起點。
- **翻譯／工具**：© 本專案作者，個人非商業性質的中文化成果。
- 同系列紀錄：[u6-cht](https://github.com/wicanr2/u6-cht)（創世紀VI 繁中化）。

> **授權**：本 repo 各部分採不同授權（引擎補丁 GPL-3.0-or-later、字型 OFL-1.1、翻譯個人非商業、文件 CC BY 4.0），詳見 [`LICENSE`](LICENSE)。
> 延伸閱讀：[引擎移植筆記 `docs/ENGINE-NOTES.md`](docs/ENGINE-NOTES.md)、[術語對照 `CONTEXT.md`](CONTEXT.md)、[Syndicate AI 邏輯考古](docs/syndicate-ai-logic.md)。
